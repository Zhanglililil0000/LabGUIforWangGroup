...
def simple_photodiode_test():
    ...
    print("每次读取100个点取平均，间隔1秒")
    print("CSV格式: 第一列=位置, 第二列=平均值(V)")
    print("-" * 50)
    ...
    for i in range(60, 260, 2):
        Delay_Stage.moveto(axis, float(i))
...
def save_simple_csv(measurement_numbers, average_values):
    ...
    # 创建只有两列的DataFrame
    df = pd.DataFrame({
        'Position': measurement_numbers,
        'Average_Voltage_V': average_values
    })
    
    # 生成文件名
    filename = f"simple_photodiode_correlation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    
    # 保存CSV（不保存索引）
    df.to_csv(filename, index=False)
    
    print(f"\n数据已保存: {filename}")
    print(f"文件格式: 第一列=位置, 第二列=平均值(V)")
